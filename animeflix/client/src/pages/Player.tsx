...
export default function Player() {
  ...
  return <div className="container min-h-[75vh] py-8 sm:py-12"><div className="mb-7 flex flex-col justify-between gap-4 sm:flex-row sm:items-center"><div><Link href={`/anime/${encodeURIComponent(id.split("-").slice(0, -1).join("-") || id)}`} className="mb-4 inline-flex items-center gap-2 text-xs font-bold uppercase tracking-[0.18em] text-white/45 transition hover:text-[#e5ff6d]"><ArrowLeft size={14} /> Back to title</Link><h1 className="font-display text-3xl font-bold tracking-[-0.05em] text-white sm:text-4xl">{title}</h1><p className="mt-2 text-sm text-white/40">Choose a server, then press play.</p></div>{servers.length ? <div className="flex flex-wrap gap-2">{servers.map((server, index) => <button key={`${server.url}-${index}`} onClick={() => setActive(server)} className={`flex items-center gap-2 rounded-full border px-4 py-2 text-xs font-bold transition ${active?.url === server.url ? "border-[#e5ff6d] bg-[#e5ff6d] text-[#11150d]" : "border-white/10 bg-white/[0.05] text-white/65 hover:border-white/25 hover:text-white"}`}><ServerIcon size={13} />{server.name || `Server ${index + 1}`}</button>)}</div> : null}</div><div className="overflow-hidden rounded-2xl border border-white/[0.1] bg-black shadow-[0_20px_70px_rgba(0,0,0,.35)]"><div className="relative aspect-video min-h-[260px] w-full">{loading ? <div className="flex h-full items-center justify-center gap-3 text-sm text-white/50"><Loader2 className="animate-spin text-[#e5ff6d]" size={20} /> Finding authorized embeds...</div> : active?.url ? <iframe title={`${title} player`} src={active.url} className="h-full w-full border-0" allow="autoplay; fullscreen; picture-in-picture" allowFullScreen /> : <div className="flex h-full flex-col items-center justify-center px-6 text-center"><div className="mb-4 flex h-14 w-14 items-center justify-center rounded-full bg-[#e5ff6d]/10 text-[#e5ff6d]"><Play size={22} /></div><h2 className="font-display text-xl font-bold text-white">No embed available</h2><p className="mt-2 max-w-md text-sm leading-6 text-white/45">The provider did not return an authorized player for this episode. Try another provider or come back later.</p></div>}</div></div><div className="mt-5 flex items-start gap-3 rounded-xl border border-[#e5ff6d]/15 bg-[#e5ff6d]/[0.05] p-4 text-xs leading-5 text-white/50"><AlertTriangle size={16} className="mt-0.5 shrink-0 text-[#e5ff6d]" /><p>AnimeFlix only displays embed URLs returned by your configured Renime API. It does not host or upload video files. Please use the player and content only where you have permission. <a href="https://github.com/itzzzDark/Renime-API" target="_blank" rel="noreferrer" className="inline-flex items-center gap-1 font-bold text-[#e5ff6d]">API docs <ExternalLink size={11} /></a></p></div></div>;
}
